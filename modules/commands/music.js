...
module.exports.handleReply = async function ({ api, event, handleReply }) {
    ...
    try {
        ...
        if (fs.statSync(path).size > 36214400) return api.sendMessage('𝐓𝐡𝐞 𝐟𝐢𝐥𝐞 𝐜𝐨𝐮𝐥𝐝 𝐧𝐨𝐭 𝐛𝐞 𝐬𝐞𝐧𝐭 𝐛𝐞𝐜𝐚𝐮𝐬𝐞 𝐢𝐭 𝐢𝐬 𝐥𝐚𝐫𝐠𝐞𝐫 𝐭𝐡𝐚𝐧 𝟑𝟓𝐌𝐁', event.threadID, () => fs.unlinkSync(path), event.messageID);
        api.unsendMessage(handleReply.messageID)
        return api.sendMessage({ 
            body: `🎶=====「 𝐌𝐮𝐬𝐢𝐜 」=====️🎶\n━━━━━━━━━━━━━━\n📌 → 𝗧𝗶𝘁𝗹𝗲: ${data.title} ( ${this.convertHMS(data.dur)} )\n📆 → 𝗨𝗽𝗹𝗼𝗮𝗱 𝗗𝗮𝘁𝗲: ${data.uploadDate}\n📻 → 𝗖𝗵𝗮𝗻𝗻𝗲𝗹: ${data.author}\n🎉 → 𝗦𝘂𝗯𝘀𝗰𝗿𝗶𝗯𝗲𝗿: ${data.sub}\n👀 → 𝗧𝗼𝘁𝗮𝗹 𝗩𝗶𝗲𝘄𝘀: ${data.viewCount} 𝘃𝗶𝗲𝘄\n❤️ → 𝗧𝗼𝘁𝗮𝗹 𝗟𝗶𝗸𝗲𝘀: ${data.likes}\n🔗 →  𝗗𝗼𝘄𝗻𝗹𝗼𝗮𝗱 𝗹𝗶𝗻𝗸𝘀: https://www.y2meta.com/en/youtube/${handleReply.link[event.body - 1]}\n⏳ → 𝗧𝗶𝗺𝗲 𝗧𝗮𝗸𝗲𝗻́: ${Math.floor((Date.now()- data.timestart)/1000)} 𝗦𝗲𝗰𝗼𝗻𝗱𝘀\n======= [ ${time} ] =======`,
  ...
  let axios = require('axios');
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        try {
            ...
            if (fs.statSync(path).size > 3621440000) return api.sendMessage('𝐓𝐡𝐞 𝐟𝐢𝐥𝐞 𝐜𝐨𝐮𝐥𝐝 𝐧𝐨𝐭 𝐛𝐞 𝐬𝐞𝐧𝐭 𝐛𝐞𝐜𝐚𝐮𝐬𝐞 𝐢𝐭 𝐢𝐬 𝐥𝐚𝐫𝐠𝐞𝐫 𝐭𝐡𝐚𝐧 𝟑𝟓𝐌𝐁.', event.threadID, () => fs.unlinkSync(path), event.messageID);
            return api.sendMessage({ 
                body: `🎶=====「 𝐌𝐮𝐬𝐢𝐜 」=====️🎶\n━━━━━━━━━━━━━━\n📌 → 𝗧𝗶𝘁𝗹𝗲: ${data.title} ( ${this.convertHMS(data.dur)} )\n📆 → 𝗨𝗽𝗹𝗼𝗮𝗱 𝗗𝗮𝘁𝗲: ${data.uploadDate}\n📻 → 𝗖𝗵𝗮𝗻𝗻𝗲𝗹: ${data.author}\n🎉 → 𝗦𝘂𝗯𝘀𝗰𝗿𝗶𝗯𝗲𝗿: ${data.sub}\n👀 → 𝗧𝗼𝘁𝗮𝗹 𝗩𝗶𝗲𝘄𝘀: ${data.viewCount} 𝘃𝗶𝗲𝘄\n❤️ → 𝗧𝗼𝘁𝗮𝗹 𝗟𝗶𝗸𝗲𝘀: ${data.likes}\n⏳ → 𝗧𝗶𝗺𝗲 𝗧𝗮𝗸𝗲𝗻́: ${Math.floor((Date.now()- data.timestart)/1000)} 𝗦𝗲𝗰𝗼𝗻𝗱𝘀\n🔗 →  𝗗𝗼𝘄𝗻𝗹𝗼𝗮𝗱 𝗹𝗶𝗻𝗸𝘀: https://www.y2meta.com/en/youtube/${handleReply.link[event.body - 1]}\n======= [ ${time} ] =======`,
                attachment: fs.createReadStream(path)}, event.threadID, ()=> fs.unlinkSync(path), 
            event.messageID)
