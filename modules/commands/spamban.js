...
module.exports.handleEvent = async function ({ Users, Threads, api, event})  {
  ...
  if ((global.client.autoban[senderID].timeStart + (timee*1000)) <= Date.now()) {
    ...
    if (global.client.autoban[senderID].number >= num) {
      ...
      api.sendMessage("😻You have been banned from using bot ib admin, please remove, depending on the case, it will be removed https://www.facebook.com/priyanshu.rajput.official\n😻ID: " + senderID + " \n😻Name: " + dataUser.name + `\n😻Reason: spam bot ${num} time/${timee}s\n\n✔️Reported to admin bot`, threadID,
    () => {
