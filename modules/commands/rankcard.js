...
module.exports.makeRankCard = async (data) => {    
  ...
  Canvas.registerFont(__root + "/urbanjungle.ttf", {
		family: "Manrope",
		weight: "urbanjungle",
		style: "normal"
	});
  ...
  let random = Math.floor(Math.random() * 15) + 1;
	...
	ctx.font = `urbanjungle 36px Manrope`;
	...
	ctx.font = `urbanjungle 35px Manrope`;
	ctx.fillStyle = getRandomColor();
	ctx.textAlign = "end";
	...
	ctx.font = `urbanjungle 32px Manrope`;
	ctx.fillStyle = getRandomColor();
	ctx.textAlign = "end";
	...
	ctx.font = `urbanjungle 26px Manrope`;
	ctx.fillStyle = getRandomColor();
	ctx.textAlign = "start";
...
module.exports.onLoad = async function () {
	...
	if (!existsSync(resolve(__dirname, 'cache', 'rankcard1.png'))) await downloadFile("https://raw.githubusercontent.com/catalizcs/storage-data/master/rank/rank_card/rankcard.png", resolve(__dirname, 'cache', 'rankcard1.png'));
}
