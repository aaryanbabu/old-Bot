module.exports.config = {
    ...
    description: "Kiss the person you want(for girls)",
...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'cache', 'hon2.png');
    if (!existsSync(dirMaterial + "")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://i.imgur.com/zEgxlno.png", path);
...
async function makeImage({ one, two }) {
    ...
    let hon_img = await jimp.read(__root + "/hon2.png");
    let pathImg = __root + `/hon2_${one}_${two}.png`;
    ...
    hon_img.resize(700, 604).composite(circleOne.resize(130, 130), 70, 400).composite(circleTwo.resize(130, 130), 300, 340);
