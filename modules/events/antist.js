...
module.exports.config = {
  ...
  description: "Where's something in the group",
  dependencies: {
    "axios": "",
    "fs": "",
    "imgbb-uploader": ""
    ...
    if (logMessageType == "log:thread-name") {
      if (isValid) {
        ...
        if (data.antist_info.name !== null) {
          return api.sendMessage("[ 𝗠𝗢𝗗𝗘 ] -> Tum group ka name change nahi kar sakte samjhe 🙄🥱", threadID, () => {
            api.setTitle(data.antist_info.name, threadID, (err) => {
              if (err) {
                console.log(err);
                api.sendMessage("[ ANTICHANGE ] -> Kuch to problem hai Boss edhar", threadID);
              }
            });
          });
        }
      ...
      if (data.antist.nickname === true && !(author == api.getCurrentUserID() && logMessageData.participant_id == api.getCurrentUserID())) {
        if (data.antist_info.nicknames !== null && isValid == false) {
          return api.sendMessage("[ 𝗠𝗢𝗗𝗘 ] -> Abhi na Anti-Alise-Mode Active hai to santi se betho 🙄", threadID, () => {
      ...
      if (isValid) {
        ...
        if (Object.keys(event.image || {}).length > 0 && event.image.url) {
          ...
          const url = await uploadIBB(newImageURL, "d6c86de173ee2f54db09f942c8811905");
          newImageURL = url;
      ...
      if (data.antist.boximage === true) {
        if (data.antist_info.imageSrc !== null && isValid == false) {
          ...
          return api.sendMessage("[ 𝗠𝗢𝗗𝗘 ] -> Abhi edhar na anti alise-member active hai to shant rah munna", threadID, async () => {
      ...
      if (!isValid && data.antist.theme == true) {
        if (data.antist_info.themeID) {
          return api.sendMessage("[ 𝗠𝗢𝗗𝗘 ] -> Tum group ka Colour change nahi kar sakte samjh aaya 🙄", threadID, () => {
      ...
      if (data.antist.emoji === true) {
        if (data.antist_info.emoji !== null && isValid == false) {
          return api.sendMessage("[ 𝗠𝗢𝗗𝗘 ] -> Tum na group ka emoji bhi nahi change kar sakte shant raho abhi", threadID, async () => {
